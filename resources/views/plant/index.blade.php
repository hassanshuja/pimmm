                                        ...
                                        <table id="simpletable" class="table table-striped table-bordered nowrap">
                                            ...
                                            @foreach($plants as $plant)
                                                <tr>
                                                    ...
                                                    @if($plant->created_by !=null)
                                                        <td>{{$plant->getCreated->first_name.' '.$plant->getCreated->last_name}}</td>
                                                    @else
                                                        <td>{{$plant->created_by}}</td>
                                                    @endif
                                                    @if($plant->updated_by !=null)
                                                        <td>{{$plant->getUpdated->first_name.' '.$plant->getUpdated->last_name}}</td>
                                                    @else
                                                        <td>{{$plant->updated_by}}</td>
                                                    @endif
                                                    {{--<td><a href="/editPlant/{{$plant->id}}"/>edit</td>--}}
                                                    <td><div class="btn-group btn-group-sm" style="float: none;"><a href="{{url('/editPlant/'.$plant->id)}}" class="btn btn-warning" >Edit</a>
                                                            <form class="myform" id="{{$plant->id}}"  method="post"  action="{{url('/deletePlant/'.$plant->id)}}">   @csrf
