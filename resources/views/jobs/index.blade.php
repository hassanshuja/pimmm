                                        ...
                                        <table id="simpletable" class="table table-striped table-bordered nowrap">
                                            ...
                                            @foreach($jobs as $job)
                                                <tr>
                                                    ...
                                                    <td>{{$job->jobnumber}}</td>
                                                    <td>{{$job->estimatedby}}</td>
                                                    <td>{{$job->jobdate}}</td>
                                                    <td>{{$job->customerpo}}</td>
                                                    <td>{{$job->entry->name}}</td>


                                                    <td><div class="btn-group btn-group-sm p8" style="float: none;">

                                                            <a href="{{url('/jobDetails/'.$job->id)}}" class="btn btn-warning" >Details</a>
                                                            <br>
                                                            <a href="{{url('/jobInfo/'.$job->id)}}" class="btn btn-warning" >Job info</a>
                                                            <br>
                                                            <a href="{{url('/editJob/'.$job->id)}}" class="btn btn-warning" >Edit</a>
                                                            <form class="myform p7" id="{{$job->id}}"  method="post"  action="{{url('/deleteJob/'.$job->id)}}">   @csrf
